...
void HariMain(void)
{
	...
	unsigned char *buf_back, buf_mouse[256], *buf_logo;
	struct SHEET *sht_back, *sht_mouse, *sht_logo;
	...
	*((int *) 0x0fe4) = (int) shtctl;
    ...
    /* sht_logo */
    sht_logo = sheet_alloc(shtctl);
    buf_logo  = (unsigned char *) memman_alloc_4k(memman, 23 * 23);
    sheet_setbuf(sht_logo, buf_logo, 23, 23, 99);
    init_logo(buf_logo);
    putblock8_8(binfo->vram, binfo->scrnx, 23, 23, 15, 3, buf_logo, 23);

	sheet_slide(sht_back,  0,  0);
	sheet_slide(key_win,   50, 50);
    sheet_slide(sht_logo,  15,  3);
	sheet_slide(sht_mouse, mx, my);
	sheet_updown(sht_back,  0);
	sheet_updown(key_win,   1);
    sheet_updown(sht_logo,  2);
